...
- **190 項 DATA.GOV.HK 資源索引配方**包含 356 組已查核的來源至數據集對應，涵蓋 350 個
  不重複的數據集識別碼。2026 年 9 月 4 日的更新把它們解析為 **5,862 項實際供應者資源**：
  5,391 個毋須參數的 HTTPS 網址、6 個需要參數的網址範本，以及 465 個安全擷取器會拒絕的
  HTTP-only 舊網址。
- 2026 年 9 月 4 日的有界限內容核查，從 **350 個數據集中的 234 個**直接檔案或 API
  收到非空白 2xx 樣本；另記錄 5 項當前供應者失敗，以及 111 個沒有免參數直接 HTTPS
  內容候選資源的數據集。這是每個數據集的代表性證據，
