...
function AdminRoutes({ operatingProfile }: { operatingProfile: OperatingProfile }) {
  ...
  return <AppShell product="HK Open Data Runtime" environment="Self-hosted" navigation={navigation} activeHref={active} utility={<span className="operator">OP <span>Local operator</span></span>}><div className="admin-profile-banner"><strong>Runtime:</strong> Self-hosted {operatingProfile} profile. Provider access is operator-controlled.</div><Routes><Route path="/" element={<OverviewPage />} /><Route path="/sources" element={<SourcesPage />} /><Route path="/sources/:sourceId" element={<SourceReviewPage />} /><Route path="/targets" element={<TargetsPage />} /><Route path="/incidents" element={<IncidentsPage />} /><Route path="/deliveries" element={<DeliveriesPage />} /><Route path="/audit" element={<AuditPage />} /><Route path="*" element={<div className="standard-page"><h1>Page not found</h1></div>} /></Routes></AppShell>;
}
