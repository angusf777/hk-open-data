...
export function SourcesPage() {
  ...
  return <div className="standard-page"><header className="page-header"><div><p className="eyebrow">SOURCE SETTINGS</p><h1>Sources</h1><p>Terms review, connector version, freshness, and enablement state.</p></div></header><div className="filter-bar"><label>Review status<select value={approval} onChange={(event) => setApproval(event.target.value)}><option value="all">All</option><option value="approved">Approved</option><option value="pending">Pending</option><option value="restricted">Restricted</option></select></label></div>{query.isPending ? <p role="status">Loading sources…</p> : <DataTable caption="Sources" rows={rows} rowKey={(item) => item.id} columns={[{ key: "id", label: "ID", render: (item) => item.id }, { key: "source", label: "Source", render: (item) => <><a href={`/sources/${item.id}`}>{item.name}</a><small>{item.provider}</small></> }, { key: "approval", label: "Review", render: (item) => <StatusLabel tone={statusTone(item.approval)}>{item.approval}</StatusLabel> }, { key: "freshness", label: "Freshness", render: (item) => <StatusDot tone={statusTone(item.freshness)}>{item.freshness}</StatusDot> }, { key: "success", label: "Last success", render: (item) => formatHkt(item.lastSuccess) }]} />}</div>;
}
